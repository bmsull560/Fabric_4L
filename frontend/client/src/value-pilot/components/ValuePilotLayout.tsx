...
const workflowSteps = [
  ...
  { path: "/workflow/driver-tree", label: "Driver Tree" },
  { path: "/workflow/evidence", label: "Evidence" },
  { path: "/workflow/calculator", label: "Calculator" },
...
const workflowItems = [
  ...
  { icon: GitFork, label: "Driver Tree", path: "/workflow/driver-tree" },
  { icon: Database, label: "Evidence", path: "/workflow/evidence" },
  { icon: Calculator, label: "Calculator", path: "/workflow/calculator" },
  { icon: FileText, label: "Value Case", path: "/workflow/value-case" },
];
...
/* ─── Component ─── */
interface ValuePilotLayoutProps {
        ...
        <div className="px-3 pb-2 space-y-0.5">
          {bottomItems.map((item) =>
            collapsed ? (
              ...
              <button key={item.label} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-sidebar-foreground/70 hover:text-sidebar-foreground hover:bg-sidebar-accent transition-colors">
                <item.icon className="w-4 h-4 shrink-0" />
