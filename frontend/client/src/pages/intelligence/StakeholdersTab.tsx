...
export default function StakeholdersTab() {
  ...
  const { messages, sendMessage, suggestedActions } = useAgentStream({ activeTab: "stakeholders", accountName: account?.name ?? "Account" });
  const stakeholders = data?.stakeholders ?? [];

  if (isLoading) return <div className="p-6 text-sm text-muted-foreground">Loading stakeholders…</div>;
  if (error || !account) return <div className="p-6 text-sm text-destructive">Failed to load stakeholders.</div>;
