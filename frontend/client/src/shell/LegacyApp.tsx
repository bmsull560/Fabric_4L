...
function AccountScopedParentRedirect({ basePath, defaultTab }: { basePath: string; defaultTab?: string }) {
  const selectedAccountId = useAccountContextStore(state => state.selectedAccountId);
  if (!selectedAccountId) {
    return <Navigate to="/accounts" />;
  }
  const suffix = defaultTab ? `/${selectedAccountId}/${defaultTab}` : `/${selectedAccountId}`;
  return <Navigate to={`${basePath}${suffix}`} />;
}
...
function AppRoutes({ tierProps, isLoading, isAuthenticated }: { tierProps: { currentTier: RequiredUserTier; effectiveTier: RequiredUserTier }; isLoading: boolean; isAuthenticated: boolean; }) {
  ...
  return (
    <>
      {WorkspaceRoutes({ tierProps, AuthenticatedRoute, Navigate, AccountContextSync, WorkspaceContextRedirect, IntelligenceRedirect, StudioRedirect, BillingRoute }, { SignalsTab, DriversTab, EvidenceTab, StakeholdersTab, EnrichmentTab, OntologyMatchTab, HypothesesTab, CompetitiveTab, ROITab, EvidenceLibraryTab, ActionPlanTab, ValueModelTab, NarrativeTab, StudioEnrichmentTab, StudioCompetitiveTab, StudioROITab, StudioEvidenceTab })}
    ...
    <Suspense fallback={<PageLoader />}>
     <Switch>
      ...
      <Route path="/hypothesis">
        <AuthenticatedRoute {...tierProps}>
          <AccountScopedParentRedirect basePath="/hypothesis" defaultTab="hypothesis" />
        </AuthenticatedRoute>
      </Route>
      ...
      <Route path="/drivers">
        <AuthenticatedRoute {...tierProps}>
          <AccountScopedParentRedirect basePath="/drivers" />
        </AuthenticatedRoute>
      </Route>
      <Route path="/drivers/:accountId">
        <AuthenticatedRoute {...tierProps}>
      ...
      <Route path="/calculator">
        <AuthenticatedRoute {...tierProps}>
          <AccountScopedParentRedirect basePath="/calculator" defaultTab="roi" />
        </AuthenticatedRoute>
      </Route>
      ...
      <Route path="/value-case">
        <AuthenticatedRoute {...tierProps}>
          <AccountScopedParentRedirect basePath="/value-case" />
        </AuthenticatedRoute>
      </Route>
      ...
      <Route path="/realization">
        <AuthenticatedRoute {...tierProps}>
          <AccountScopedParentRedirect basePath="/realization" />
        </AuthenticatedRoute>
      </Route>
