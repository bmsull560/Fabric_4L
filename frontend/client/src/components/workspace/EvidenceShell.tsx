 ...
 *
 * Provides the shared layout for all Evidence workspace tabs:
 *   Evidence → Alternatives → Solution Cost
 *
 * Layout:
 ...
 *   - "Value Calculator" CTA in header
 */
...
function AccountHeader({ account, accountId }: { account: AccountContext; accountId: string }) {
  ...
  return (
    <div className="h-11 bg-muted/50 border-b border-border flex items-center px-6 gap-3 text-[12px] shrink-0">
      ...
      <Btn
        variant="primary"
        onClick={() => navigate(`/calculator/${accountId}`)}
        className="gap-1.5"
      >
        <ArrowRight size={13} />
        Value Calculator
...
function WorkspaceTabs({ accountId, activeTab }: { accountId: string; activeTab: string }) {
  return (
    <div className="flex border-b border-border px-6" role="tablist">
      {TABS.map((tab) => (
        <Link key={tab.key} href={`/evidence/${accountId}/${tab.key}`}>
          <button
            role="tab"
