...
class EvaluateFormulaTool(BaseTool):
    ...
    async def execute(self, input_data: EvaluateFormulaInput) -> EvaluateFormulaOutput:
        ...
        try:
            ...
            return EvaluateFormulaOutput(
                result=None,
                substituted_formula=input_data.formula,
                success=False,
                error=str(e)
            )
...
class CalculateROITool(BaseTool):
    ...
    def _approximate_irr(self, investment: float, returns: List[float]) -> Optional[float]:
    ...
    def _calculate_payback(self, investment: float, returns: List[float]) -> Optional[float]:
