...
class SendNotificationTool(BaseTool):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.from_email = config.get("from_email", "notifications@valuefabric.io") if config else "notifications@valuefabric.io"
        # Slack config
...
class CreateTaskTool(BaseTool):
    ...
    async def _create_asana_task(self, client: httpx.AsyncClient, input_data: CreateTaskInput) -> CreateTaskOutput:
    ...
    async def _create_monday_task(self, client: httpx.AsyncClient, input_data: CreateTaskInput) -> CreateTaskOutput:
        ...
        if "data" in data and "create_item" in data["data"]:
            ...
            return CreateTaskOutput(task_id="", success=False, url="", error=str(data.get("errors")))
    
    async def _create_clickup_task(self, client: httpx.AsyncClient, input_data: CreateTaskInput) -> CreateTaskOutput:
...
class ScheduleMeetingTool(BaseTool):
    ...
    async def _schedule_google_meeting(self, client: httpx.AsyncClient, input_data: ScheduleMeetingInput) -> ScheduleMeetingOutput:
        ...
        payload = {
            ...
            "conferenceData": {"createRequest": {"requestId": f"meet-{datetime.utcnow().timestamp()}"}}
        }
    ...
    async def _schedule_outlook_meeting(self, client: httpx.AsyncClient, input_data: ScheduleMeetingInput) -> ScheduleMeetingOutput:
        ...
        payload = {
            ...
            "start": {"dateTime": start_time, "timeZone": "UTC"},
            "end": {"dateTime": end_time.isoformat(), "timeZone": "UTC"},
            "attendees": [{"emailAddress": {"address": email}, "type": "required"} for email in input_data.attendees]
        }
...
class ExportToCRMTool(BaseTool):
    ...
    async def _export_to_salesforce(self, client: httpx.AsyncClient, input_data: ExportToCRMInput) -> ExportToCRMOutput:
    ...
    async def _export_to_hubspot(self, client: httpx.AsyncClient, input_data: ExportToCRMInput) -> ExportToCRMOutput:
        """Export to HubSpot."""
        prospect_id = input_data.prospect_id
        entity_type = input_data.entity_type
        ...
        if response.status_code in [200, 201]:
            record_id = data.get("engagement", {}).get("id") if "engagement" in data else data.get("id")
            return ExportToCRMOutput(
