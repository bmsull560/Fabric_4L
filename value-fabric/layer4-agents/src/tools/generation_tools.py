...
Maximum {max_length} words.""",
    ...
    async def _call_llm(self, prompt: str, max_tokens: int = 1000) -> str:
        ...
        response = await client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "You are a professional business writer creating business case content."},
    ...
    async def execute(self, input_data: GenerateSectionInput) -> GenerateSectionOutput:
        ...
        template = self.SECTION_TEMPLATES.get(input_data.section_type, self.SECTION_TEMPLATES["executive_summary"])
...
class CreateChartTool(BaseTool):
    ...
    async def execute(self, input_data: CreateChartInput) -> CreateChartOutput:
        ...
        if chart_type == "bar":
            ...
            chart_data["slices"] = [
                {"label": d.get("label", ""), "value": d.get("value", 0)}
                for d in data
            ]
