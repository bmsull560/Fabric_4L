...
class StateDiffResponse(BaseModel):
    ...
    timestamp_a: str
    timestamp_b: str
    added_fields: List[str] = Field(default_factory=list, description="Fields present in B but not A")
    removed_fields: List[str] = Field(default_factory=list, description="Fields present in A but not B")
...
class ResumeFromCheckpointRequest(BaseModel):
    ...
    skip_nodes: Optional[List[str]] = Field(
        None,
...
async def list_checkpoints(
    ...
    if executor.checkpoint_saver is None:
        raise HTTPException(
            status_code=503,
            detail="Checkpointing not configured - cannot retrieve checkpoints"
        )
...
async def get_checkpoint_state(
    ...
    if executor.checkpoint_saver is None:
        raise HTTPException(
            status_code=503,
            detail="Checkpointing not configured"
        )
    
    try:
        state_data = await _get_checkpoint_data(
            executor.checkpoint_saver,
            workflow_id,
            checkpoint_id
        )
...
async def compare_checkpoints(
    ...
    try:
        ...
        state_a = await _get_checkpoint_data(
            executor.checkpoint_saver,
            workflow_id,
            request.checkpoint_a_id
        )
        state_b = await _get_checkpoint_data(
            executor.checkpoint_saver,
            workflow_id,
            request.checkpoint_b_id
        )
        ...
        if not state_a or not state_b:
            ...
            raise HTTPException(
                status_code=404,
                detail=f"Checkpoint(s) not found: {', '.join(missing)}"
            )
        
        # Calculate diff
        diff = _calculate_state_diff(
            state_a.get("state", {}),
            state_b.get("state", {})
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to compare checkpoints: {str(e)}"
        )
...
async def resume_from_checkpoint(
    ...
    if executor.checkpoint_saver is None:
        raise HTTPException(
            status_code=503,
            detail="Checkpointing not configured - cannot resume from checkpoint"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to resume from checkpoint: {str(e)}"
        )
