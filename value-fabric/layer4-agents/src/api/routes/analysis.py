...
from ...models.agent_state import ROIInputData, WhitespaceInputData, BusinessCaseInputData
...
class ROIAnalysisRequest(BaseModel):
    ...
    prospect_data: Dict[str, float] = Field(default_factory=dict, description="Prospect-specific variables")
...
class BusinessCaseRequest(BaseModel):
    ...
    sections: List[str] = Field(default_factory=lambda: [
        "executive_summary", "roi_analysis", "implementation", "next_steps"
    ])
...
async def quick_whitespace_analysis(
    ...
    try:
        ...
        return WhitespaceAnalysisResponse(
            prospect_id=request.prospect_id,
            extracted_needs=result.output_data.get("analyze_prospect", {}).get("extracted_needs", []),
