...
async def get_performance_metrics(
    ...
    if node_metrics:
        ...
        nodes_with_errors = [
            m.node_name for m in node_metrics if m.error_count > 0
        ]
    else:
...
def _calculate_depth(value: Any, max_depth: int = 5, current_depth: int = 0) -> int:
    ...
    if isinstance(value, dict):
        if not value:
            return current_depth
        return max(
            _calculate_depth(v, max_depth, current_depth + 1)
...
def _detect_error_pattern(errors: List[str]) -> Optional[str]:
...
def _infer_changed_fields(node_name: Optional[str]) -> List[str]:
