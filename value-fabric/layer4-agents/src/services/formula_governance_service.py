    ...
    async def get_governance(self, formula_id: str) -> Optional[FormulaGovernance]:
        ...
        async with self._driver.session() as session:
            ...
            for v in versions_data:
                if v:
                    versions.append(FormulaVersion(
            ...
            for d in record["outgoing_deps"]:
                if d["id"]:
                    all_deps.append(FormulaDependency(
            ...
            return FormulaGovernance(
                ...
                last_reviewed_at=datetime.fromisoformat(f["lastReviewedAt"]) if "lastReviewedAt" in f else None,
                next_review_at=datetime.fromisoformat(f["nextReviewAt"]) if "nextReviewAt" in f else None,
            )
        ...
        async with self._driver.session() as session:
            ...
            for r in records:
                ...
                versions.append(FormulaVersion(
                    ...
                    created_at=datetime.fromisoformat(fv["createdAt"]) if "createdAt" in fv else datetime.now(timezone.utc),
