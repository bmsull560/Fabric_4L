...
class InputField(BaseModel):
    ...
    field_type: str = Field(default="text", description="Input type: text, number, select, boolean, textarea")
    ...
    help_text: Optional[str] = Field(None, description="Additional help/instructions")
...
class PauseContext(BaseModel):
    ...
    previous_outputs: Dict[str, Any] = Field(default_factory=dict, description="Outputs from previous nodes")
    error_details: Optional[str] = Field(None, description="Error message if pause due to failure")
...
class PausePoint(BaseModel):
    ...
    help_text: Optional[str] = Field(None, description="Additional guidance for user")
    required_inputs: List[InputField] = Field(default_factory=list, description="Fields needed to resume")
    validation_schema: Dict[str, Any] = Field(default_factory=dict, description="JSON Schema for resume_data")
    context: Optional[PauseContext] = Field(None, description="Execution context")
    estimated_wait_seconds: Optional[int] = Field(None, description="Seconds until auto-resume (if any)")
    auto_resume_conditions: Optional[Dict[str, Any]] = Field(None, description="Conditions for auto-resumption")
    expires_at: Optional[datetime] = Field(None, description="Pause expiration time")
    ...
    def to_resume_prompt(self) -> str:
        ...
        if self.required_inputs:
            ...
            for field in self.required_inputs:
                ...
                lines.append(f"- **{field.label}**{req_marker}: {field.help_text or field.field_type}")
...
class PausePointTemplate(BaseModel):
    ...
    help_text_template: Optional[str] = Field(None, description="Help text with {placeholders}")
    ...
    def instantiate(
        ...
        custom_inputs: Optional[List[InputField]] = None
    ) -> PausePoint:
        ...
        help_text = self.help_text_template.format(**placeholders) if self.help_text_template else None
