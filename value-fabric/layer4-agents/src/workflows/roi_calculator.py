...
class ROICalculatorWorkflow(BaseWorkflow):
    ...
    async def _execute_tool(self, tool_name: str, state: ROIAgentState, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _execute_substitute_variables(self, state: ROIAgentState) -> Dict[str, Any]:
        ...
        defaults = query_result.get("results", [{}])[0].get("defaults", {}) if query_result.get("results") else {}
        
        variables = {
            "annual_revenue": prospect_data.get("annual_revenue", defaults.get("annual_revenue", 10000000)),
            "employee_count": prospect_data.get("company_size", defaults.get("employee_count", 500)),
    ...
    async def _execute_aggregate_roi(self, state: ROIAgentState) -> Dict[str, Any]:
        ...
        payback_months = (investment / (total_annual_value / 12)) if total_annual_value > 0 else None
        ...
        aggregated = {
            ...
            "average_confidence": sum(r.get("confidence", 0) for r in calc_results) / len(calc_results),
        }
