...
class WhitespaceAnalysisWorkflow(BaseWorkflow):
    ...
    async def _execute_tool(self, tool_name: str, state: WhitespaceAgentState, config: Dict[str, Any]) -> Dict[str, Any]:
...
Return ONLY the JSON array, no other text."""
        ...
        return {
            "extracted_needs": extracted_needs if isinstance(extracted_needs, list) else [extracted_needs],
    ...
    async def _execute_identify_gaps(self, state: WhitespaceAgentState) -> Dict[str, Any]:
        ...
        for need in needs:
            ...
            if best_score >= 0.7:
                ...
                gap_type = "coverage"
                impact = "medium"
                recommendation = f"Partial match - explore {best_match.get('name', 'N/A')} with customizations"
            else:
                gap_type = "capability"
                impact = "high"
        ...
        return {
            ...
            "gap_count": len(gaps),
            "coverage_percentage": sum(1 for g in gaps if g.gap_type == "none") / len(gaps) * 100 if gaps else 0
        }
    ...
    async def _execute_score_opportunity(self, state: WhitespaceAgentState) -> Dict[str, Any]:
        ...
        if coverage < 50:
            recommendations.append("Significant whitespace exists - consider custom solution approach")
        elif coverage > 80:
