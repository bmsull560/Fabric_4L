...
from sqlalchemy import (
    ...
    ForeignKey, JSON, Index, UniqueConstraint, Numeric, Float
)
...
class RawContent(Base):
    ...
    # Processing Status
    processing_status = Column(String(50), default="PENDING")  # PENDING, EXTRACTING, EXTRACTED, FAILED
...
class ExtractedData(Base):
    ...
    provenance_extracted_at = Column(DateTime(timezone=True), default=datetime.utcnow, nullable=False)
...
class RobotsTxtCache(Base):
    ...
    organization_id = Column(UUID(as_uuid=True), nullable=False, index=True)  # Added for multi-tenancy
...
class CrawlQueueItem(Base):
    ...
    status = Column(String(50), default="PENDING", index=True)  # PENDING, PROCESSING, COMPLETED, FAILED, RETRYING
