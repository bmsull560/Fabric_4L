...
class FinancialFact:
    ...
    value: Any
    unit: Optional[str] = None  # e.g., "USD", "shares"
...
class FinancialStatement:
    ...
    def get_fact(self, concept_pattern: str) -> Optional[FinancialFact]:
...
class ParsedXBRL:
    ...
    document_type: Optional[str] = None  # 10-K, 10-Q, etc.
    document_period: Optional[datetime] = None
...
class XBRLParser:
    ...
    def _build_financial_statements(self, facts: List[FinancialFact]) -> ParsedXBRL:
        ...
        for fact in facts:
            ...
            if any(x in concept_lower for x in ['asset', 'liabilit', 'equity', 'cash', 'inventory', 'receivable']):
                balance_facts.append(fact)
            # Income statement indicators
            elif any(x in concept_lower for x in ['revenue', 'income', 'expense', 'profit', 'loss', 'ebit', 'cost']):
                income_facts.append(fact)
            # Cash flow indicators
            elif any(x in concept_lower for x in ['cashflow', 'cash.flow', 'operating', 'investing', 'financing']):
    ...
    def _build_statement(self, statement_type: str, facts: List[FinancialFact]) -> FinancialStatement:
    ...
    def _extract_key_metrics(self, facts: List[FinancialFact]) -> Dict[str, Any]:
        ...
        for metric_name, concept_patterns in self.CONCEPT_MAPPINGS.items():
            for pattern in concept_patterns:
                matching_facts = [
                    f for f in facts
                    if pattern.lower() in f.concept.lower()
                ]
    ...
    def _extract_entity(self, root: ET.Element) -> Optional[str]:
    ...
    def _extract_document_type(self, root: ET.Element) -> Optional[str]:
