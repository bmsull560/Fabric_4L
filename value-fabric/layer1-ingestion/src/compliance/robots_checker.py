...
class RobotsChecker:
    ...
    async def _get_robots_txt(
        ...
        try:
            ...
            self.logger.error(
                "HTTP error fetching robots.txt",
                domain=domain,
    ...
    def _get_cached_robots_txt(self, domain: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            with get_db_session() as session:
                cache_entry = session.query(RobotsTxtCache).filter(
                    RobotsTxtCache.domain == domain,
                    RobotsTxtCache.expires_at > datetime.now(timezone.utc)
                ).first()
            ...
            self.logger.error(
                "Failed to get cached robots.txt",
                domain=domain,
                error=str(e)
            )
            return None
    ...
    def _cache_robots_txt(
        ...
        try:
            with get_db_session() as session:
                ...
                existing = session.query(RobotsTxtCache).filter(
                    RobotsTxtCache.domain == domain
                ).first()
