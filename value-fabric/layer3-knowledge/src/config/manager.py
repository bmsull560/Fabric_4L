...
class APIConfig(BaseModel):
    ...
    description: str = Field(default="Value Fabric Layer 3 Knowledge Graph API", description="API description")
...
class ConfigurationManager:
    ...
    def load_config(self) -> AppConfig:
        ...
        for source in self.config_sources:
            try:
                ...
                if source.required:
                    raise ValueError(f"Failed to load required configuration source '{source.name}': {e}")
                else:
                    logger.warning(f"Failed to load optional configuration source '{source.name}': {e}")
    ...
    def _log_config_summary(self) -> None:
        ...
        logger.info(f"Database: {self._config.database.host}:{self._config.database.port}/{self._config.database.database}")
    ...
    def _remove_sensitive_data(self, data: Dict[str, Any]) -> None:
        ...
        """
        sensitive_keys = {
            "password", "secret_key", "api_key", "access_key", "secret_key", "auth_token"
        }
...
def load_config(environment: Optional[Environment] = None) -> AppConfig:
