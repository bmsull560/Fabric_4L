...
class MetricsConfig:
    ...
    def __init__(
        ...
        self.default_buckets = default_buckets or [0.1, 0.25, 0.5, 1.0, 2.5, 5.0, 10.0, 25.0, 50.0, 100.0, 250.0, 500.0, 1000.0]
...
class PrometheusMetrics:
    ...
    def __init__(self, config: Optional[MetricsConfig] = None):
        ...
        if not PROMETHEUS_AVAILABLE:
            raise ImportError("prometheus_client library is required for PrometheusMetrics")
    ...
    def set_memory_usage(
        ...
        self._metrics["memory_usage_bytes"].labels(
            component=component
        ).set(bytes_used)
    ...
    def set_cpu_usage(
        ...
        self._metrics["cpu_usage_percent"].labels(
            component=component
        ).set(percent)
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if operation == "database":
                    ...
                    metrics.increment_database_operations(db_operation, database, status)
                    metrics.observe_database_operation_duration(duration, db_operation, database)
                elif operation == "cache":
                    cache_type = labels.get("cache_type", "redis")
                    cache_operation = labels.get("cache_operation", "get")
                    metrics.increment_cache_operations(cache_operation, cache_type, status)
                    ...
                    metrics.observe_search_query_duration(duration, search_type, entity_type)
