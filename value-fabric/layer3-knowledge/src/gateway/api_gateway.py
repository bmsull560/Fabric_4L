...
class GatewayConfig(BaseModel):
    ...
    protocol: GatewayProtocol = Field(default=GatewayProtocol.HTTP, description="Gateway protocol")
    ...
    max_response_size: int = Field(default=10485760, description="Max response size in bytes")
...
class ServiceMeshConfig(BaseModel):
    ...
    protocol: ServiceMeshProtocol = Field(default=ServiceMeshProtocol.ISTIO, description="Service mesh protocol")
    control_plane_url: str = Field(default="http://istio-control-plane:15000", description="Control plane URL")
    ...
    async def unregister_service(self, service_name: str, endpoint_id: str):
        ...
        if service_name in self.local_registry:
            self.local_registry[service_name] = [
                ep for ep in self.local_registry[service_name]
...
class RouteEngine:
    ...
    def match_route(self, request_path: str, request_method: str, headers: Dict[str, str], query_params: Dict[str, str]) -> Optional[RouteRule]:
        ...
        for route in self.routes:
            ...
            if self._match_conditions(route, request_path, request_method, headers, query_params):
                return route
        
        return None
    
    def _match_conditions(self, route: RouteRule, request_path: str, request_method: str, headers: Dict[str, str], query_params: Dict[str, str]) -> bool:
    ...
    def check_fault_injection(self, service_name: str) -> Optional[FaultInjection]:
...
class APIGateway:
    ...
    def __init__(self, gateway_config: GatewayConfig, service_mesh_config: ServiceMeshConfig):
    ...
    async def handle_request(self, request_path: str, request_method: str, headers: Dict[str, str], query_params: Dict[str, str], body: Optional[bytes] = None) -> Tuple[int, Dict[str, str], bytes]:
        ...
        try:
            # Match route
            route = self.route_engine.match_route(request_path, request_method, headers, query_params)
            
            if not route:
                return 404, {"Content-Type": "application/json"}, b'{"error": "Route not found"}'
            
            # Get service endpoints
            endpoints = await self.service_registry.get_service_endpoints(route.target_service)
            ...
            if not circuit_breaker.call_allowed():
                return 503, {"Content-Type": "application/json"}, b'{"error": "Circuit breaker open"}'
            ...
            if fault:
                if fault.fault_type == "delay" and fault.delay_ms:
                    ...
                    return fault.error_code, {"Content-Type": "application/json"}, fault.error_message.encode() if fault.error_message else b'{"error": "Service fault injected"}'
            
            # Forward request
            response = await self._forward_request(endpoint, request_path, request_method, headers, query_params, body, route)
    ...
    async def _forward_request(self, endpoint: ServiceEndpoint, request_path: str, request_method: str, headers: Dict[str, str], query_params: Dict[str, str], body: Optional[bytes], route: RouteRule) -> Tuple[int, Dict[str, str], bytes]:
...
class ServiceMesh:
    ...
    def __init__(self, config: ServiceMeshConfig, gateway: APIGateway):
        ...
        self.control_plane_client: Optional[httpx.AsyncClient] = None
