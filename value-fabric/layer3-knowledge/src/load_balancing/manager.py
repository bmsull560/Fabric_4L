...
class BackendServer:
    ...
    def record_error(self, error: Optional[str] = None):
...
class LoadBalancerConfig(BaseModel):
    ...
    strategy: LoadBalanceStrategy = Field(default=LoadBalanceStrategy.ROUND_ROBIN, description="Load balancing strategy")
    ...
    session_affinity_timeout: int = Field(default=3600, description="Session affinity timeout in seconds")
...
class AutoScalingConfig(BaseModel):
    ...
    policy: ScalingPolicy = Field(default=ScalingPolicy.CPU_BASED, description="Scaling policy")
    ...
    scale_up_threshold: float = Field(default=80.0, description="Scale up threshold percentage")
    scale_down_threshold: float = Field(default=20.0, description="Scale down threshold percentage")
    ...
    target_cpu_utilization: float = Field(default=70.0, description="Target CPU utilization")
    target_memory_utilization: float = Field(default=80.0, description="Target memory utilization")
    target_response_time: float = Field(default=500.0, description="Target response time in ms")
    target_request_rate: int = Field(default=1000, description="Target request rate per minute")
...
class LoadBalancer:
    ...
    async def select_backend(self, client_ip: Optional[str] = None, session_id: Optional[str] = None) -> Optional[BackendServer]:
        ...
        if self.config.enable_sticky_sessions and session_id:
            if session_id in self.session_affinity:
                ...
                if backend_id in self.backends and self.backends[backend_id].is_available:
    ...
    def _ip_hash_select(self, backends: List[BackendServer], client_ip: Optional[str]) -> BackendServer:
        """IP hash selection."""
        if not client_ip:
    ...
    async def _check_backend_health(self, client: httpx.AsyncClient, backend: BackendServer):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                backend.last_health_check = datetime.utcnow()
                logger.warning(f"Backend {backend.id} health check failed: {response.status_code}")
...
class AutoScaler:
    ...
    async def _determine_scaling_action(self, metrics: Dict[str, Any]) -> ScaleDirection:
    ...
    def _evaluate_response_time_scaling(self, metrics: Dict[str, Any]) -> ScaleDirection:
    ...
    def _evaluate_queue_length_scaling(self, metrics: Dict[str, Any]) -> ScaleDirection:
        ...
        if queue_length > max_queue_per_instance * len(self.load_balancer.backends):
            return ScaleDirection.UP
        elif queue_length < max_queue_per_instance * len(self.load_balancer.backends) * 0.3:
            return ScaleDirection.DOWN
    ...
    def _can_scale_up(self) -> bool:
        ...
        cooldown_passed = (datetime.utcnow() - last_scale).total_seconds() > self.config.scale_up_cooldown
    ...
    def _can_scale_down(self) -> bool:
        ...
        cooldown_passed = (datetime.utcnow() - last_scale).total_seconds() > self.config.scale_down_cooldown
        return cooldown_passed
    
    async def _execute_scaling(self, direction: ScaleDirection, metrics: Dict[str, Any]):
    ...
    async def _scale_down(self, metrics: Dict[str, Any]):
        ...
        available_backends = [
            backend for backend in self.load_balancer.backends.values()
            if backend.current_connections == 0 and backend.health_status == HealthStatus.HEALTHY
        ]
...
class LoadBalancingSystem:
    ...
    def __init__(self, load_balancer_config: LoadBalancerConfig, auto_scaling_config: AutoScalingConfig):
    ...
    async def handle_request(self, client_ip: Optional[str] = None, session_id: Optional[str] = None) -> Optional[BackendServer]:
...
_load_balancing_system: Optional[LoadBalancingSystem] = None


def get_load_balancing_system() -> Optional[LoadBalancingSystem]:
...
async def initialize_load_balancing(
    ...
    _load_balancing_system = LoadBalancingSystem(load_balancer_config, auto_scaling_config)
