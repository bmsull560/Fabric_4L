...
class AnalyticsQuery(BaseModel):
    ...
    granularity: TimeGranularity = Field(default=TimeGranularity.HOUR, description="Time granularity")
...
class AnalyticsConfig(BaseModel):
    ...
    max_events_per_second: int = Field(default=10000, description="Max events per second")
    sampling_rate: float = Field(default=1.0, description="Event sampling rate")
    enable_real_time: bool = Field(default=True, description="Enable real-time analytics")
    enable_historical: bool = Field(default=True, description="Enable historical analytics")
...
class MetricsCollector:
    ...
    def collect_event(self, event: AnalyticsEvent):
        ...
        """
        # Apply sampling
        if self.config.sampling_rate < 1.0 and hash(event.event_id) % 100 > self.config.sampling_rate * 100:
            return
        ...
        # Data volume metrics
        self.counters["data_volume_bytes"] += event.request_size_bytes + event.response_size_bytes
    ...
    def get_metric(self, metric_name: str, aggregation: AggregationType = AggregationType.SUM) -> Any:
...
class AnalyticsStore:
    ...
    async def store_event(self, event: AnalyticsEvent, ttl: int = 86400):
        ...
        try:
            ...
            await self.redis_client.hincrbyfloat(time_key, "response_time", event.response_time_ms)
        ...
        try:
            ...
            while current_time <= end_time:
                ...
                if granularity == TimeGranularity.MINUTE:
                    ...
                    time_bucket = current_time.replace(hour=0, minute=0, second=0, microsecond=0)
                else:
                    time_bucket = current_time.replace(second=0, microsecond=0)
                ...
                if time_data:
                    data_point = {
                        ...
                        "avg_response_time": float(time_data.get("response_time", 0)) / max(1, int(time_data.get("total", 1)))
                    }
    ...
    async def get_top_endpoints(self, start_time: datetime, end_time: datetime, limit: int = 10) -> List[Dict[str, Any]]:
...
class AnalyticsManager:
    ...
    async def _execute_query(self, query: AnalyticsQuery) -> List[Dict[str, Any]]:
        ...
        if query.metric == "api_requests_total":
            ...
            data = await self.store.get_top_endpoints(start_time, end_time, query.limit or 10)
        else:
            data = []
        
        return data
    
    async def _get_requests_data(self, query: AnalyticsQuery, start_time: datetime, end_time: datetime) -> List[Dict[str, Any]]:
        """Get requests data.
    ...
    async def _get_response_time_data(self, query: AnalyticsQuery, start_time: datetime, end_time: datetime) -> List[Dict[str, Any]]:
    ...
    async def _get_success_rate_data(self, query: AnalyticsQuery, start_time: datetime, end_time: datetime) -> List[Dict[str, Any]]:
        ...
        for i, widget in enumerate(dashboard.widgets):
            if widget["type"] == "metric":
                ...
                if widget["metric"] == "top_endpoints":
                    ...
                    data = await self.store.get_top_endpoints(start_time, end_time, widget.get("limit", 10))
    ...
    async def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_response_time": self.collector.get_metric("api_response_time_ms", AggregationType.AVERAGE),
