...
class RateLimitRequest(BaseModel):
    ...
    identifier: str = Field(..., description="Unique identifier (user ID, API key, IP, etc.)")
...
class RateLimitResponse(BaseModel):
    ...
    rule_id: Optional[str] = Field(None, description="Applied rule ID")
    action: Optional[RateLimitAction] = Field(None, description="Action taken")
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Additional metadata")
...
class RateLimitConfig(BaseModel):
    ...
    default_limits: List[RateLimitRule] = Field(default_factory=list, description="Default rate limits")
    custom_limits: List[RateLimitRule] = Field(default_factory=list, description="Custom rate limits")
    cache_ttl: int = Field(default=3600, description="Cache TTL in seconds")
    cleanup_interval: int = Field(default=300, description="Cleanup interval in seconds")
    max_requests_per_window: int = Field(default=10000, description="Max requests per window")
...
class RateLimitManager:
    ...
    def _match_conditions(self, conditions: Dict[str, Any], request: RateLimitRequest) -> bool:
    ...
    async def _check_rule(self, rule: RateLimitRule, request: RateLimitRequest) -> RateLimitResponse:
        """Check specific rule.
    ...
    async def _check_token_bucket(self, rule: RateLimitRule, key: str, request: RateLimitRequest) -> RateLimitResponse:
    ...
    async def _check_sliding_window(self, rule: RateLimitRule, key: str, request: RateLimitRequest) -> RateLimitResponse:
        ...
        if count <= rule.limit:
            ...
            ttl = await self.store.redis_client.ttl(key) if self.store.redis_client else rule.window_seconds
    ...
    async def _check_fixed_window(self, rule: RateLimitRule, key: str, request: RateLimitRequest) -> RateLimitResponse:
    ...
    async def _check_leaky_bucket(self, rule: RateLimitRule, key: str, request: RateLimitRequest) -> RateLimitResponse:
    ...
    async def _check_adaptive(self, rule: RateLimitRule, key: str, request: RateLimitRequest) -> RateLimitResponse:
