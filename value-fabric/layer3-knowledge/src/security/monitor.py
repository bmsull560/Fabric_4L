...
class SecurityAlert(BaseModel):
    ...
    signature_id: Optional[str] = Field(None, description="Detection signature ID")
...
class SecurityConfig(BaseModel):
    ...
    anomaly_threshold: float = Field(default=2.0, description="Anomaly detection threshold (std deviations)")
    max_events_per_second: int = Field(default=1000, description="Max events per second")
    geo_ip_enabled: bool = Field(default=True, description="Enable GeoIP lookup")
    reputation_enabled: bool = Field(default=True, description="Enable IP reputation lookup")
    ml_enabled: bool = Field(default=False, description="Enable ML-based detection")
    auto_block_enabled: bool = Field(default=False, description="Enable automatic blocking")
...
class AnomalyDetector:
    ...
    def __init__(self, config: SecurityConfig):
        ...
        self.user_patterns: Dict[str, Dict[str, Any]] = defaultdict(lambda: defaultdict(list))
        self.ip_patterns: Dict[str, Dict[str, Any]] = defaultdict(lambda: defaultdict(list))
        self.endpoint_patterns: Dict[str, Dict[str, Any]] = defaultdict(lambda: defaultdict(list))
    ...
    def add_event(self, event: SecurityEvent):
        ...
        if event.user_id:
            ...
            self.user_patterns[event.user_id]["response_times"].append(event.response_time_ms)
            self.user_patterns[event.user_id]["request_sizes"].append(event.request_size)
        ...
        self.ip_patterns[event.source_ip]["response_times"].append(event.response_time_ms)
        ...
        self.endpoint_patterns[event.endpoint]["response_times"].append(event.response_time_ms)
    ...
    def detect_anomalies(self, event: SecurityEvent) -> List[Dict[str, Any]]:
        ...
        if event.user_id:
            recent_user_events = [e for e in self.recent_events if e.user_id == event.user_id]
        ...
        if event.endpoint:
            recent_endpoint_events = [e for e in self.recent_events if e.endpoint == event.endpoint]
            unique_ips = set(e.source_ip for e in recent_endpoint_events)
            if len(unique_ips) > len(recent_endpoint_events) * 0.8:  # Most requests from different IPs
                anomalies.append({
...
class SignatureMatcher:
    ...
    def _load_default_signatures(self):
        ...
        default_signatures = [
            ...
            ThreatSignature(
                ...
                conditions={"endpoint": "/auth/login", "max_attempts": 10, "time_window": 300}
            ),
            ThreatSignature(
    ...
    def _threshold_match(self, event: SecurityEvent, signature: ThreatSignature) -> bool:
        ...
        if "max_response_size" in conditions and event.response_size > conditions["max_response_size"]:
            return True
        
        # Check response time threshold
        if "max_response_time" in conditions and event.response_time_ms > conditions["max_response_time"]:
            return True
        
        return False
    
    def _signature_match(self, event: SecurityEvent, signature: ThreatSignature) -> bool:
...
class SecurityStore:
    ...
    async def get_alerts_by_status(self, status: AlertStatus, limit: int = 100) -> List[SecurityAlert]:
...
class SecurityMonitor:
    ...
    async def process_event(self, event: SecurityEvent) -> List[SecurityAlert]:
        ...
        for alert in alerts:
            ...
            if self.config.auto_block_enabled and alert.threat_level in [ThreatLevel.HIGH, ThreatLevel.CRITICAL]:
    ...
    def _create_signature_alert(self, event: SecurityEvent, signature: ThreatSignature) -> SecurityAlert:
        ...
        return SecurityAlert(
            ...
            risk_score=self._calculate_risk_score(signature.severity, signature.false_positive_rate),
            metadata={"signature_name": signature.name}
        )
    
    def _create_anomaly_alert(self, event: SecurityEvent, anomaly: Dict[str, Any]) -> SecurityAlert:
    ...
    def _calculate_risk_score(self, threat_level: ThreatLevel, false_positive_rate: float) -> float:
    ...
    async def get_security_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_signatures": len([s for s in self.signature_matcher.signatures.values() if s.enabled]),
            "recent_alerts": len(recent_alerts),
            "alert_levels": {
                "low": len([a for a in recent_alerts if a.threat_level == ThreatLevel.LOW]),
                "medium": len([a for a in recent_alerts if a.threat_level == ThreatLevel.MEDIUM]),
                "high": len([a for a in recent_alerts if a.threat_level == ThreatLevel.HIGH]),
                "critical": len([a for a in recent_alerts if a.threat_level == ThreatLevel.CRITICAL])
            }
        }
