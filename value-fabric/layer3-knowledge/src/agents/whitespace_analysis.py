...
class WhitespaceAnalysisAgent(BaseAgent):
        ...
        async with self._driver.session() as session:
            for pain_id in pain_points:
                result = await session.run(
                    missing_query,
                    {"pain_id": pain_id, "existing_capabilities": existing_capabilities},
                )
    ...
    async def _generate_expansion_pathways(
        self,
        prospect_company: str,
        target_capability_id: Optional[str],
    ) -> Dict[str, Any]:
        ...
        async with self._driver.session() as session:
            result = await session.run(expansion_query, {"cap_id": target_capability_id})
            async for record in result:
                pathways.append({
                    ...
                    "estimated_value": self._estimate_value(record["value_drivers"]),
                })
    ...
    def _calculate_average_maturity(
    ...
    def _generate_insights(self, gaps: List[Dict]) -> List[str]:
        ...
        if gaps:
            insights.append(f"Identified {len(gaps)} capability gaps requiring attention")
