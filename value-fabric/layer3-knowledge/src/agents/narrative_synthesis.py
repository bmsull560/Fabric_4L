...
""",
        ...
        "slide_deck_standard_v3": {
            ...
            "slides": [
                ...
                {"slide_number": 2, "layout": "executive_summary", "title": "Executive Summary", "content": "{executive_summary}"},
                {"slide_number": 3, "layout": "challenges", "title": "Current State Challenges", "content": "{challenges}"},
                {"slide_number": 4, "layout": "solution", "title": "Proposed Solution", "content": "{solution}"},
                {"slide_number": 5, "layout": "financial", "title": "Financial Analysis", "content": "{financial_analysis}"},
                {"slide_number": 6, "layout": "sensitivity", "title": "Sensitivity Analysis", "content": "{sensitivity_analysis}"},
                {"slide_number": 7, "layout": "roadmap", "title": "Implementation Roadmap", "content": "{roadmap}"},
                {"slide_number": 8, "layout": "risks", "title": "Risk Mitigation", "content": "{risks}"},
                {"slide_number": 9, "layout": "next_steps", "title": "Recommended Next Steps", "content": "{next_steps}"},
            ],
        },
        "risk_proposal_v1": {
...
""",
    ...
    TEMPLATE_FILTERS = {
        ...
        'month_year': lambda x: x.strftime("%B %Y") if hasattr(x, 'strftime') else str(x),
    }
    ...
    async def execute(self, context: Dict[str, Any]) -> AgentResult:
        ...
        try:
            ...
            if operation == "generate_executive_summary":
                ...
                result = await self._generate_slide_deck(
                    title, data, template_id
                )
            elif operation == "generate_proposal":
                result = await self._generate_risk_proposal(
                    title, data, template_id
                )
            else:
    ...
    async def _generate_executive_summary(
        ...
        """
        template = self.templates.get(template_id) or self.templates["executive_summary_v2"]
    ...
    async def _generate_slide_deck(
        ...
        for slide_def in template_def.get("slides", []):
            ...
            slides.append({
                ...
                "title": self._substitute_template(slide_def["title"], formatted_data),
                "content": slide_content,
                "notes": self._generate_speaker_notes(slide_def["layout"], formatted_data),
            })
    ...
    async def _generate_risk_proposal(
        ...
        """
        template = self.templates.get(template_id) or self.templates.get("risk_proposal_v1")
    ...
    def _apply_filters(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            if isinstance(value, dict):
                ...
                if any(x in key.lower() for x in ['cost', 'investment', 'revenue', 'savings', 'price', 'value', 'roi']):
                    result[key] = self.TEMPLATE_FILTERS['currency'](value)
                elif any(x in key.lower() for x in ['rate', 'percentage', 'percent', 'growth']):
    ...
    def _extract_metrics(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        metric_keys = [
            'total_investment', 'expected_roi', 'payback_period',
            'net_present_value', 'internal_rate_of_return',
            'total_savings', 'revenue_increase', 'cost_reduction',
        ]
    ...
    def _generate_speaker_notes(
        self, layout: str, data: Dict[str, Any]
    ) -> str:
