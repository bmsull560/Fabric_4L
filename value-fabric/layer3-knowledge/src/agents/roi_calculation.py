...
class ROICalculationAgent(BaseAgent):
    ...
    async def _execute_formula(
        ...
        formula = await self._get_formula(formula_id)
        if not formula:
        ...
        for variable, values in variable_ranges.items():
            ...
            for value in values:
                ...
                try:
                    ...
                    logger.warning(f"Sensitivity test failed for {variable}={value}: {e}")
            ...
            if variable_results:
                ...
                results[variable] = {
                    ...
                    "sensitivity_ratio": (
                        ...
                        (max(values) - min(values)) if len(values) > 1 and max(values) != min(values) else 0
                    ),
    ...
    async def _retrieve_formulas(self, use_case_id: str) -> Dict[str, Any]:
    ...
    async def _get_formula(self, formula_id: str) -> Optional[FormulaNode]:
    ...
    def _validate_inputs(
        ...
        required_vars = {v["name"] for v in formula.variables if v.get("required", True)}
        ...
        for var in formula.variables:
            ...
            for test_val in test_values:
                ...
                try:
                    ...
                    results.append(float(result) if isinstance(result, Decimal) else result)
                except Exception:
                    results.append(None)
            ...
            if all(r is not None for r in results):
                ...
                sensitivities[var_name] = {
                    "elasticity": (delta_output / results[1]) / (delta_input / base_value) if results[1] and base_value else 0,
                    "absolute_sensitivity": delta_output / delta_input if delta_input else 0,
                }
