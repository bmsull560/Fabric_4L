...
class VersionedResponse(BaseModel):
    ...
    warnings: List[str] = Field(default_factory=list, description="Backward compatibility warnings")
    deprecated: Optional[Dict[str, Any]] = Field(None, description="Deprecation information if applicable")


class DeprecationInfo(BaseModel):
    """Deprecation information for API features."""
    
    deprecated_since: str = Field(..., description="Version when feature was deprecated")
    removal_version: Optional[str] = Field(None, description="Version when feature will be removed")
    ...
    sunset_date: Optional[datetime] = Field(None, description="When feature will be removed")
...
class VersionCompatibility:
    ...
    def check_deprecation(self, version: str, feature_path: str) -> Optional[DeprecationInfo]:
    ...
    def transform_response(
        ...
        if transformer:
            try:
                ...
                logger.warning(f"Response transform failed for {version}:{endpoint}: {e}")
                return data
        ...
        async def versioned_handler(request: Request) -> Response:
            ...
            if self.deprecated:
                ...
                if self.deprecated.removal_version:
                    response.headers["X-Removal-Version"] = self.deprecated.removal_version
...
class VersionMiddleware:
    ...
    async def __call__(self, request: Request, call_next):
        ...
        response.headers["X-Supported-Versions"] = ",".join(self.compatibility.supported_versions)
...
_version_compatibility: Optional[VersionCompatibility] = None
