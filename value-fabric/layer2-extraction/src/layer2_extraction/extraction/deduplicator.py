...
class EntityDeduplicator:
    ...
    async def _deduplicate_type(
        self, entity_type: str, entities: list[BaseModel]
    ) -> list[BaseModel]:
    ...
    def _merge_into_canonical(self, canonical: BaseModel, entities: list[BaseModel]) -> None:
        ...
        confidences = [getattr(e, "confidence", 0) for e in entities]
    ...
    def _merge_capability_fields(self, canonical: Capability, entities: list[BaseModel]) -> None:
    ...
    def _merge_usecase_fields(self, canonical: UseCase, entities: list[BaseModel]) -> None:
    ...
    def _merge_persona_fields(self, canonical: Persona, entities: list[BaseModel]) -> None:
    ...
    def _merge_valuedriver_fields(self, canonical: ValueDriver, entities: list[BaseModel]) -> None:
    ...
    def _merge_feature_fields(self, canonical: Feature, entities: list[BaseModel]) -> None:
        """Merge feature-specific fields."""
        # Keep the most specific technical spec
        for entity in entities:
            if isinstance(entity, Feature):
                if entity.technical_spec and len(entity.technical_spec) > len(
                    canonical.technical_spec or ""
                ):
...
async def deduplicate_entities(
    ...
    """
    deduplicator = EntityDeduplicator(api_key=api_key, similarity_threshold=similarity_threshold)
    return await deduplicator.deduplicate(
        entities, relationships=relationships, enable_coreference=enable_coreference
    )
