...
class EventStore:
    ...
    def get_since(self, last_event_id: Optional[str] = None) -> List[Dict[str, Any]]:
...
class PipelineWebSocketManager:
    ...
    async def broadcast_stage_progress(
        ...
        overall_percent = int(
            ((stage_number - 1) + stage_percent / 100) / total_stages * 100
        )
    ...
    async def broadcast_ingestion_status(
        ...
        relationships_loaded: Optional[int] = None,
        ...
        return await self.broadcast_to_job(
            ...
            payload=payload,
            message=f"Ingestion {status}" + (f" (retry {retry_count}/{max_retries})" if retry_count > 0 else "")
        )
...
_ws_manager: Optional[PipelineWebSocketManager] = None
