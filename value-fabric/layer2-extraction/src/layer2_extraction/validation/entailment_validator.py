...
CARDINALITY_CONSTRAINTS: dict[PredicateType, tuple[int | None, int | None]] = {
...
class EntailmentValidator:
    ...
    def validate(
        self, result: ExtractionResult, relationships: list[Relationship]
    ...
    def _get_entity_by_id(self, entity_id: str, result: ExtractionResult) -> Any | None:
        """Find entity by ID across all types."""
        return result.get_entity_by_id(entity_id)

    def _validate_required_properties(self, result: ExtractionResult) -> list[ValidationResult]:
                ...
                if not entity.name or len(entity.name) < 1:
                    results.append(
                        ValidationResult(
                            ...
                            passed=False,
                            severity=ValidationSeverity.ERROR,
                            message="Required property 'name' missing or empty for Capability",
                ...
                if not entity.name or len(entity.name) < 1:
                    results.append(
                        ValidationResult(
                            ...
                            passed=False,
                            severity=ValidationSeverity.ERROR,
                            message="Required property 'name' missing or empty for Feature",
                            affected_entities=[entity.id],
