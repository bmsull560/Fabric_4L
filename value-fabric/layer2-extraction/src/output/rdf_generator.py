...
class RDFGenerator:
    ...
    def _add_ontology_definitions(self) -> None:
        ...
        self.graph.add((VF.Capability, RDFS.comment, Literal(
        ...
        self.graph.add((VF.Feature, RDFS.comment, Literal(
        ...
        predicates = [
            ...
            (VF.capabilityRequiresCapability, "capability_requires_capability", "Capability requires another capability"),
            (VF.semanticallyEquivalent, "semantically_equivalent", "Entities are semantically equivalent"),
    ...
    def _add_relationship(self, relationship: Relationship) -> None:
        ...
        if predicate:
            ...
            self.graph.add((statement, VF.confidence, Literal(relationship.confidence, datatype=XSD.float)))
            ...
            if relationship.impact_level:
                self.graph.add((statement, VF.impactLevel, Literal(relationship.impact_level.value)))
            
            if relationship.strength:
                self.graph.add((statement, VF.strength, Literal(relationship.strength, datatype=XSD.float)))
            
            # Provenance
            self._add_provenance(statement, relationship.extraction_job_id, relationship.extracted_at)
