...
## Canonical Coverage Status Vocabulary

This matrix is not launch evidence and does not close launch gates. Gate closure still requires retained artifacts, logs, redacted environment metadata, and owner sign-off from the relevant approved environment.

| Status | Meaning |
|---|---|
| `covered` | Repo-owned executable coverage exists for the core workflow path and is mapped to a current suite. |
| `partially_covered` | Some executable coverage exists, but important workflow steps, edge cases, roles, or negative paths still need tests. |
| `missing` | No executable coverage has been identified for the workflow group. |
| `requires_live_environment` | Evidence depends on CI, staging, production-like services, retained artifacts, or operational monitors. |
| `requires_provider_or_external_integration` | Evidence depends on external provider, SSO/OIDC, CRM, billing, LLM, or other sandbox/live integration systems. |

## Canonical Six-Suite Workflow Coverage Matrix

The product-validation taxonomy maps to six execution suites:
...
Coverage sources referenced by this matrix include `apps/web/e2e/journeys/`, `tests/backend_integrated/`, `tests/security/`, `tests/integration/`, `apps/web/e2e/security/`, and `tests/performance/k6/`.

| ID | Workflow Category | Validation Suite | Coverage Status | Executable Coverage | Primary Gap | Next Action |
|---|---|---|---|---|---|---|
| 1 | Authentication, Tenant, and Workspace Access | Security/Tenant Isolation | `partially_covered` | `apps/web/e2e/journeys/j0-auth-session.spec.ts`; `apps/web/e2e/security/`; `tests/security/`; `tests/integration/test_tenant_isolation_end_to_end.py` | MFA, password reset, session expiry, SSO/OIDC, and hostile role edge cases need retained proof. | Add focused auth and OIDC coverage; keep SSO evidence external. |
| 2 | Account and Prospect Setup | Golden Path E2E | `partially_covered` | `apps/web/e2e/journeys/j1-golden-path-backend-integrated.spec.ts`; `j6-account-prospect-lifecycle.spec.ts`; `j23-prospect-create-redirect-signals.spec.ts`; `j24-account-intake-create-workspace.spec.ts` | Duplicate detection, merge, archive/delete, audit history, and CRM import remain incomplete. | Extend account lifecycle coverage and map CRM import to external integration evidence. |
| 3 | Data Ingestion Workflows - Layer 1 | Layer-by-Layer Validation | `partially_covered` | `apps/web/e2e/journeys/j1-ingestion-to-value-tree.spec.ts`; `j10-layer-ui-validation.spec.ts`; `tests/backend_integrated/test_cross_layer_data_flow_validation.py`; `tests/performance/k6/journey-load-test.js` | Large file, unsupported file, duplicate file, retry/cancel, and provenance edge cases need deeper proof. | Add L1 ingestion edge-case tests and operational retry assertions. |
| 4 | Extraction and Signal Detection - Layer 2 | Layer-by-Layer Validation | `partially_covered` | `apps/web/e2e/journeys/j2-intelligence-workspace.spec.ts`; `j10-layer-ui-validation-deep.spec.ts`; `tests/backend_integrated/test_cross_layer_data_flow_validation.py` | Approve/reject/edit/merge signal lifecycle and conflicting evidence paths are incomplete. | Add signal lifecycle tests with source-grounding assertions. |
| 5 | Knowledge Graph and Context Engine - Layer 3 | Layer-by-Layer Validation | `partially_covered` | `apps/web/e2e/journeys/j10-layer-ui-validation.spec.ts`; `j10-layer-ui-validation-deep.spec.ts`; `tests/backend_integrated/test_cross_layer_data_flow_validation.py`; `tests/security/test_knowledge_tools_tenant_isolation.py` | Entity correction, graph query errors, provenance on edges, and graph refresh need broader coverage. | Add graph CRUD/error/refresh tests with tenant-scoped assertions. |
| 6 | Value Pack Selection and Governance | Calculation and Evidence Integrity | `partially_covered` | `apps/web/e2e/journeys/j14-value-pack-governance.spec.ts`; `tests/integration/test_pack_integration.py` | Pack version publishing/deprecation and historical value-case preservation need tests. | Extend pack governance tests around versioning and override behavior. |
| 7 | Prospect Intelligence Workflow | Golden Path E2E | `partially_covered` | `apps/web/e2e/journeys/j2-intelligence-workspace.spec.ts`; `j24-account-signals-evidence-driver-calculator-business-case.spec.ts` | Agent explain, promote/reject intelligence, refresh after new sources, and export need proof. | Add intelligence action coverage and export assertions. |
| 8 | Stakeholder Mapping | Golden Path E2E | `partially_covered` | `apps/web/e2e/journeys/j13-stakeholder-mapping.spec.ts`; `apps/web/e2e/personas/persona-journeys.spec.ts` | Evidence-source validation and stakeholder-specific agent messaging are incomplete. | Add stakeholder evidence and role-specific messaging tests. |
| 9 | Hypothesis Generation Workflow - Layer 4 | Agent Grounding and Governance | `partially_covered` | `apps/web/e2e/journeys/j9-agent-grounding-governance.spec.ts`; `j9-agent-grounding-deep.spec.ts`; `tests/backend_integrated/test_agent_grounding_real_tool_contracts.py` | Live-provider grounding, unsupported-hypothesis rejection, and assumption labeling require stronger proof. | Add structured hypothesis lifecycle assertions; retain live provider proof separately. |
| 10 | Value Driver Tree Workflow | Calculation and Evidence Integrity | `partially_covered` | `apps/web/e2e/journeys/j7-value-realization-and-calculation.spec.ts`; `j24-account-signals-evidence-driver-calculator-business-case.spec.ts`; `tests/backend_integrated/test_calculation_evidence_provenance_integrity.py` | Driver CRUD, lever mapping, priority changes, and lineage edge cases need coverage. | Extend value-driver tree tests with evidence and formula mappings. |
| 11 | Evidence Matching Workflow | Calculation and Evidence Integrity | `partially_covered` | `apps/web/e2e/journeys/j7-calculation-evidence-deep.spec.ts`; `j24-account-signals-evidence-driver-calculator-business-case.spec.ts`; `tests/backend_integrated/test_calculation_evidence_provenance_integrity.py` | Manual evidence CRUD, conflicting evidence, and confidence/source filters need more proof. | Add evidence library lifecycle and provenance-chain tests. |
| 12 | Benchmark and Ground Truth Workflow - Layers 5 and 6 | Calculation and Evidence Integrity | `partially_covered` | `apps/web/e2e/journeys/j7-value-realization-and-calculation.spec.ts`; `j10-layer-ui-validation.spec.ts`; `tests/backend_integrated/test_calculation_evidence_provenance_integrity.py` | Benchmark governance, stale benchmark warnings, and truth-object approval/rejection need coverage. | Add benchmark policy and ground-truth lifecycle tests. |
| 13 | Formula Selection and Calculation Workflow | Calculation and Evidence Integrity | `partially_covered` | `apps/web/e2e/journeys/j7-calculation-evidence-deep.spec.ts`; `j7-value-realization-and-calculation.spec.ts`; `tests/performance/k6/formula-evaluation.js`; `tests/backend_integrated/test_calculation_evidence_provenance_integrity.py` | Formula option comparison, unit/currency/time-period validation, and lock/unlock paths need coverage. | Add formula edge-case and reproducibility assertions. |
| 14 | Value Calculator Workflow | Calculation and Evidence Integrity | `partially_covered` | `apps/web/e2e/journeys/j7-calculation-evidence-deep.spec.ts`; `j7-value-realization-and-calculation.spec.ts`; `tests/performance/k6/journey-load-test.js` | Custom scenario CRUD, sensitivity analysis, advanced metrics, and missing-data warnings need proof. | Extend calculator journey tests and performance smoke assertions. |
| 15 | Business Case Generation Workflow | Golden Path E2E | `partially_covered` | `apps/web/e2e/journeys/j3-value-studio-deliverable.spec.ts`; `j11-golden-path-business-lifecycle.spec.ts`; `j24-account-signals-evidence-driver-calculator-business-case.spec.ts`; `tests/backend_integrated/test_backend_integrated_golden_path.py` | Audience templates, claim-to-evidence validation, export formats, and regeneration compare paths need proof. | Add business-case template/export and traceability assertions. |
| 16 | Narrative and Proposal Workflow | Agent Grounding and Governance | `partially_covered` | `apps/web/e2e/journeys/j15-narrative-proposal.spec.ts`; `j9-agent-grounding-governance.spec.ts` | Version restore, unsupported claim removal, and tone/audience changes need stronger coverage. | Add narrative versioning and claim-grounding tests. |
| 17 | Agentic Chat and Right-Rail Workflow | Agent Grounding and Governance | `requires_provider_or_external_integration` | `apps/web/e2e/journeys/j9-agent-grounding-governance.spec.ts`; `j9-agent-grounding-deep.spec.ts`; `tests/backend_integrated/test_agent_grounding_real_tool_contracts.py` | Real LLM provider behavior, citations, refusal, and recommendation application require sandbox/live evidence. | Run provider-backed agent grounding suite and attach retained artifacts. |
| 18 | Review, Approval, and Governance Workflow | Security/Tenant Isolation | `partially_covered` | `apps/web/e2e/journeys/j8-approval-review-gates.spec.ts`; `j8-approval-review-deep.spec.ts`; `tests/backend_integrated/test_approval_export_crm_governance.py` | Admin policy configuration, review comments, and resubmission lifecycle need deeper coverage. | Extend approval-policy tests and export-blocking assertions. |
| 19 | Versioning, Audit, and Traceability | Security/Tenant Isolation | `partially_covered` | `apps/web/e2e/journeys/j4-governance-trust.spec.ts`; `tests/security/test_audit_event_emission.py`; `tests/security/test_privileged_audit.py` | Version comparison/restore and full source-lineage export need tests. | Add version history and immutable audit export coverage. |
| 20 | Collaboration Workflow | Operational Resilience | `partially_covered` | `apps/web/e2e/journeys/j16-collaboration.spec.ts`; `apps/web/e2e/personas/persona-journeys.spec.ts` | Real-time update behavior, conflict handling, and notifications need coverage. | Add collaboration conflict and notification tests. |
| 21 | CRM and External System Workflow | Golden Path E2E | `requires_provider_or_external_integration` | `apps/web/e2e/journeys/j17-crm-integration.spec.ts`; `apps/web/e2e/integrations/crm-external-integrations.spec.ts`; `tests/backend_integrated/test_approval_export_crm_governance.py` | CRM provider sandbox, field mapping failure, permissions, and pushback evidence require external integration proof. | Run CRM sandbox workflow with retained sync logs. |
| 22 | Value Realization Workflow | Calculation and Evidence Integrity | `partially_covered` | `apps/web/e2e/journeys/j7-value-realization-and-calculation.spec.ts`; `apps/web/e2e/personas/persona-journeys.spec.ts` | Post-sale conversion, realized-value variance, cadence, and renewal proof narrative need tests. | Extend realization plan and outcome-tracking coverage. |
| 23 | Search and Retrieval Workflow | Layer-by-Layer Validation | `partially_covered` | `apps/web/e2e/journeys/j18-search-retrieval.spec.ts`; `tests/security/test_knowledge_tools_tenant_isolation.py`; `tests/security/test_cross_tenant_api.py` | Tenant-wide search, role filters, document/evidence search, and correct-context landing need more coverage. | Add search result context and RBAC assertions. |
| 24 | Notifications and Task Workflow | Operational Resilience | `partially_covered` | `apps/web/e2e/journeys/j19-notifications-tasks.spec.ts`; `j16-collaboration.spec.ts` | Ingestion/extraction/review notification triggers and overdue task filtering need proof. | Add notification producer/consumer coverage. |
| 25 | Admin Configuration Workflow | Security/Tenant Isolation | `partially_covered` | `apps/web/e2e/journeys/j20-admin-configuration.spec.ts`; `tests/integration/test_tenant_provisioning.py`; `tests/security/test_rbac.py` | Full role/user management, retention policy, tenant health, and security-event review need tests. | Extend admin lifecycle and policy configuration coverage. |
| 26 | Security and Compliance User Workflows | Security/Tenant Isolation | `partially_covered` | `apps/web/e2e/security/tenant-isolation-validation.spec.ts`; `apps/web/e2e/journeys/j5-tier-gated-security.spec.ts`; `tests/security/`; `tests/integration/test_cross_layer_tenant_isolation.py` | PII, prompt-injection-in-document, retention deletion, and export restrictions need retained workflow proof. | Add hostile content and deletion-policy workflow coverage. |
| 27 | Error, Empty State, and Recovery Workflows | Operational Resilience | `partially_covered` | `apps/web/e2e/journeys/j12-resilience-error-recovery.spec.ts`; `tests/backend_integrated/test_operational_resilience_real_services.py`; `tests/integration/test_transaction_rollback.py` | Partial workflows, unsaved work protection, service-specific recovery, and resume paths need more coverage. | Add recovery/resume tests with explicit failure injection. |
| 28 | Full End-to-End Golden Path | Golden Path E2E | `requires_live_environment` | `apps/web/e2e/journeys/j11-golden-path-business-lifecycle.spec.ts`; `j1-golden-path-backend-integrated.spec.ts`; `tests/backend_integrated/test_backend_integrated_golden_path.py`; `.github/workflows/backend-integrated-reproducibility.yml` | Approved CI/staging retained artifacts and owner sign-off are required before this can be release evidence. | Run approved CI/staging reproducibility workflow and attach retained artifacts. |
| 29 | Full End-to-End Adversarial Path | Security/Tenant Isolation | `requires_provider_or_external_integration` | `apps/web/e2e/journeys/j22-adversarial-e2e.spec.ts`; `tests/security/test_injection.py`; `tests/security/test_p1_12_prompt_delimiters.py`; `tests/security/test_owasp_top10_complete.py` | Live/provider refusal, prompt-injection isolation, and cross-tenant leak checks need sandbox-backed evidence. | Run adversarial provider/security suite with retained artifacts. |
| 30 | Persona-Based Validation Journeys | Golden Path E2E | `partially_covered` | `apps/web/e2e/journeys/j21-persona-journeys.spec.ts`; `apps/web/e2e/personas/persona-journeys.spec.ts` | Persona-specific edge cases, executive buyer view, and cross-role handoffs need deeper proof. | Expand persona journeys and map role-specific acceptance checks. |
