...
- **Long term:** Rename or consolidate. Options:
  ...
  2. Merge root modules into `value-fabric/shared/` and delete the root package.
  3. Make `value-fabric/shared/` the only `shared` package and move repo-wide utilities elsewhere.

## Consequences
- **Risk:** A developer editing `shared/security/middleware.py` at the repo root may believe they are hardening the application, but Layer 2+ runtime imports `value-fabric/shared/security/middleware.py`.
