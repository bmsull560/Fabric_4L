...
| Structured logging with correlation IDs | ✅ PASS | 2026-05-12 | `packages/shared/src/value_fabric/shared/observability/logging.py`, `packages/shared/src/value_fabric/shared/observability/probes.py` | JSON structured logs include correlation/request IDs and are available to layers via shared logging utilities. |
| Observability smoke gate | ✅ PASS | 2026-05-12 | `.github/workflows/prod-readiness.yml` (observability-readiness job), `scripts/ops/release-gate.sh` (obs gate) | Release-candidate and mainline profiles require green observability gate execution. |
