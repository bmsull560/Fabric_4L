"""Software Value Pack - Integrity Tests
...
class TestPackManifest:
    ...
    def test_workflow_has_pack_id(self, pack_files):
        ...
        assert pack_files["workflow_template"]["pack_id"] == "software-v1"
...
class TestNamingConventions:
    ...
    def test_entity_ids_follow_convention(self, pack_data):
        """Entity IDs must follow sw-{type}-{number} pattern."""
        for entity in pack_data["ontology"]["entities"]:
            entity_id = entity["id"]
            assert entity_id.startswith("sw-"), f"Entity {entity_id} doesn't start with sw-"
    ...
    def test_formula_ids_follow_convention(self, pack_data):
        """Formula IDs must follow sw-f-{number} pattern."""
        for formula in pack_data["formulas"]["formulas"]:
            formula_id = formula["formula_id"]
            assert formula_id.startswith("sw-f-"), f"Formula {formula_id} doesn't start with sw-f-"
    ...
    def test_variable_ids_follow_convention(self, pack_data):
        """Variable IDs must follow sw-var-{number} pattern."""
        for variable in pack_data["variables"]["variables"]:
            var_id = variable["variable_id"]
            assert var_id.startswith("sw-var-"), f"Variable {var_id} doesn't start with sw-var-"
