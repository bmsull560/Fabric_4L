"""Software Value Pack - Ontology Relationship Tests
...
class TestSoftwareSpecificEntities:
    """Verify Software-specific entity content."""
    ...
    def test_has_ai_ml_capability(self, ontology):
        """Must have AI/ML Platform capability."""
        caps = [e for e in ontology["entities"] if e["type"] == "Capability"]
        ai_caps = [c for c in caps if "AI" in c["name"] or "ML" in c["name"]]
        assert len(ai_caps) > 0, "Missing AI/ML capability"

    def test_has_developer_platform_capability(self, ontology):
        """Must have Developer Platform capability."""
        caps = [e for e in ontology["entities"] if e["type"] == "Capability"]
        dev_caps = [c for c in caps if "Developer" in c["name"] or "Platform" in c["name"]]
        assert len(dev_caps) > 0, "Missing Developer Platform capability"

    def test_has_churn_prevention_usecase(self, ontology):
        """Must have Churn Prevention use case."""
        usecases = [e for e in ontology["entities"] if e["type"] == "UseCase"]
        churn_ucs = [uc for uc in usecases if "Churn" in uc["name"]]
        assert len(churn_ucs) > 0, "Missing Churn Prevention use case"

    def test_has_cro_persona(self, ontology):
        """Must have Chief Revenue Officer or similar executive."""
        personas = [e for e in ontology["entities"] if e["type"] == "Persona"]
        exec_personas = [p for p in personas if "Revenue" in p["name"] or "CRO" in p["name"]]
        assert len(exec_personas) > 0, "Missing CRO/CRO persona"

    def test_has_nrr_valuedriver(self, ontology):
        """Must have Net Revenue Retention value driver."""
        drivers = [e for e in ontology["entities"] if e["type"] == "ValueDriver"]
        nrr_drivers = [d for d in drivers if "NRR" in d["name"] or "Retention" in d["name"]]
        assert len(nrr_drivers) > 0, "Missing NRR value driver"
