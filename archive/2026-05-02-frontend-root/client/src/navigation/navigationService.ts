 ...
 * Used for building safe regex patterns from path templates.
 */
...
export function buildPath(pathTemplate: string, params: NavigationParams = {}): string {
  ...
  // Normalize the path to handle edge cases from removed params
...
export function resolveWorkspacePath(path: string, accountId: string | null): string {
  ...
  for (const prefix of ACCOUNT_SCOPED_PREFIXES) {
    if (path === prefix) {
      return `${prefix}/${encodedAccountId}`;
    }
    if (path.startsWith(`${prefix}/`)) {
      const suffix = path.slice(prefix.length + 1);
      return `${prefix}/${encodedAccountId}/${suffix}`;
    }
  }

  // Legacy studio routes
  if (path === '/studio') return `/studio/${encodedAccountId}`;
  if (path.startsWith('/studio/')) {
    const suffix = path.slice('/studio/'.length);
    return `/studio/${encodedAccountId}/${suffix}`;
  }
...
export function pathToState(path: string): { state: RouteState | null; params: NavigationParams } {
  ...
  for (const [state, config] of Object.entries(ROUTE_MAP)) {
    ...
    // Split by "/" and process each segment separately to properly escape static parts
    const segments = config.path.split('/').filter(s => s.length > 0);
    const escapedSegments = segments.map(segment => {
      if (segment.startsWith(':')) {
        // Parameter segment - use capture group
        return '([^/]+)';
      }
