...
root_conftest = module_from_spec(_SPEC)
...
def test_skip_reason_contains_command_and_categories(dependency_key: str) -> None:
...
def test_check_postgres_contract_with_fake_psycopg2(monkeypatch: pytest.MonkeyPatch) -> None:
    fake_connect = Mock()
...
def test_check_redis_contract_with_fake_client(monkeypatch: pytest.MonkeyPatch) -> None:
    fake_client = Mock()
...
def test_check_neo4j_contract_with_fake_driver(monkeypatch: pytest.MonkeyPatch) -> None:
    fake_driver = Mock()
