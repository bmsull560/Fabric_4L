...
export function useBilling(customerId: string) {
  ...
  const openCustomerPortal = async (returnUrl: string) => {
    ...
    if (result.url) {
      window.location.href = result.url; // navigation-guardrail: ignore - external Stripe portal redirect
    } else {
  ...
  const subscribe = async (planId: string, successUrl: string, cancelUrl: string) => {
    ...
    if (result.url) {
      window.location.href = result.url; // navigation-guardrail: ignore - external Stripe checkout redirect
    } else {
