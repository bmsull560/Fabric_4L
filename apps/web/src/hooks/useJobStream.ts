...
const JobStreamEventTypeSchema = z.enum(['progress', 'status', 'log', 'entity', 'complete', 'error']);
...
const JobIdSchema = z.string().min(1).regex(/^[a-zA-Z0-9_-]+$/);
...
export function useJobStream(jobId: string | null) {
  ...
  useEffect(() => {
    ...
    const trySseConnection = (): void => {
      ...
      try {
        ...
        logError('Failed to build SSE URL', { jobId: effectJobId, error: String(err) });
        startPolling();
        return;
      }
      ...
      timeoutRef.current = setTimeout(() => {
        ...
        if (eventSourceRef.current === es && isMountedRef.current) {
          logWarn('SSE connection timeout, falling back to polling', { jobId: effectJobId });
          cleanup();
      ...
      es.onmessage = (event: MessageEvent): void => {
        try {
          ...
          const parsed = parseJobStreamEvent(parsedData);
          ...
          setState(prev => {
            switch (parsed.type) {
              case 'progress':
                return { 
                  ...prev, 
                  progress: typeof parsed.data === 'number' ? parsed.data : prev.progress 
                };
              
              case 'status':
                return { 
                  ...prev, 
                  status: mapJobStatus(typeof parsed.data === 'string' ? parsed.data : '') 
                };
              ...
              case 'log': {
                ...
                return {
                  ...prev,
                  logs: [...prev.logs, validatedLog.success ? validatedLog.data : {
                    timestamp: '',
                    level: 'INFO',
                    message: '',
                  }],
              ...
              case 'entity': {
                ...
                return {
                  ...prev,
                  entities: [...prev.entities, validatedEntity.success ? validatedEntity.data : {
                    type: 'unknown',
                    name: 'Unknown',
                  }],
                };
      ...
      es.onerror = (): void => {
        ...
        if (eventSourceRef.current === es) {
          logWarn('SSE connection error, falling back to polling', { jobId: effectJobId });
          setIsConnected(false);
          sseSupportedRef.current = false;
          startPolling();
        }
        ...
        try {
          ...
          logWarn('Error closing failed EventSource', { error: String(closeErr) });
        }
      };
    };
  ...
  // ============================================================================
  const isStreaming = isConnected && (state.status === 'pending' || state.status === 'running');
