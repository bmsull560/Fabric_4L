...
timeout "$STACK_TIMEOUT" compose up -d --build --wait
...
echo "→ release-smoke: running backend-integrated release smoke pytest suite (timeout=$PYTEST_TIMEOUT)"
...
timeout "$PYTEST_TIMEOUT" "$PYTEST_BIN" \
  tests/backend_integrated/test_release_environment_smoke_validation.py \
  -m release_smoke \
  -v \
  --tb=short \
  --junitxml="$ARTIFACT_DIR/release_smoke_junit.xml" \
  | tee "$ARTIFACT_DIR/logs/release_smoke_pytest.log"
