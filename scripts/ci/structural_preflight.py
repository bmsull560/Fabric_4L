...
def check_namespace_shadowing(repo_root: Path) -> list[Finding]:
    """Detect root shared/ shadowing value-fabric/shared/."""
    findings = []
    ...
    if root_shared.exists() and vf_shared.exists():
        findings.append(Finding(
            ...
            message="Both root shared/ and value-fabric/shared/ exist",
            recommendation="Merge root shared/ into value-fabric/shared/ or remove root shared/",
        ))
    ...
    if exit_code == 0:
        ...
        if "value-fabric" not in resolved_path and "value_fabric" not in resolved_path:
            findings.append(Finding(
                ...
                recommendation="Ensure shared resolves to canonical location in value-fabric/",
            ))
