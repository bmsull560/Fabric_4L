...
def validate_summary(artifact_dir: Path) -> dict[str, Any]:
    ...
    require(isinstance(summary["detail"], str) and summary["detail"].strip(), "summary detail must be non-empty")
...
def validate_manifest(artifact_dir: Path, expected_status: str) -> dict[str, Any]:
    ...
    for entry in manifest["entries"]:
        ...
        require(isinstance(entry["bytes"], int) and entry["bytes"] >= 0, "manifest entry bytes must be non-negative integer")
        entry_paths.add(entry["path"])
    for required in ("live-workflow-validation-summary.md", "live-workflow-validation-summary.json", "artifact-manifest.json", "docker-compose.live.resolved.yml"):
...
def validate_endpoint_probes(artifact_dir: Path, *, required_when_present: bool) -> None:
    ...
    for index, line in enumerate(lines[1:], start=2):
        ...
        require(columns[2].isdigit() and len(columns[2]) == 3, f"endpoint probe line {index} has invalid status code: {columns[2]!r}")
...
def validate_seed_report(artifact_dir: Path, *, required: bool) -> None:
    ...
    require(seed["status"] in {"present", "partial", "blocked", "missing"}, f"invalid seed report status: {seed['status']!r}")
...
def main() -> int:
    ...
    parser.add_argument("artifact_dir", nargs="?", default="artifacts/live-workflow-validation", help="Artifact directory to validate")
