...
if [ -s "$GATE_RESULT_REL" ]; then
    GIT_SHA="$($_PYTHON -c "import json,sys; d=json.load(open('$GATE_RESULT_REL')); print(d.get('git_sha','${GIT_SHA}'))" 2>/dev/null || echo "$GIT_SHA")"
...
---
*Generated by \`scripts/ops/render-release-summary.sh\`*
EOF
