...
async def get_value_case(account_id: str, tenant_id: str = Depends(tenant_required)):
    cases = db.business_cases.list(tenant_id=tenant_id, filter_fn=lambda c: c.account_id == account_id)
    if not cases:
        raise HTTPException(status_code=404, detail="No value case found for account")
    return cases[0]


@router.post("/value-case/generate", response_model=BusinessCase)
async def generate_value_case(account_id: str, case: BusinessCase, tenant_id: str = Depends(tenant_required)):
...
async def update_value_case(value_case_id: str, fields: dict, tenant_id: str = Depends(tenant_required)):
