...
def test_development_permits_missing_prod_required_dependency(caplog: pytest.LogCaptureFixture) -> None:
    caplog.set_level(logging.WARNING)
    verify_startup_dependencies(
        [
            DependencyRule(
                module="missing_required_dependency_for_dev_tests",
                required_in_prod=True,
                remediation="pip install missing_required_dependency_for_dev_tests",
            )
        ],
        environment="development",
    )
    assert "missing_required_dependency_for_dev_tests" in caplog.text


def test_layer2_startup_dependency_catalog_supports_dev_mode() -> None:
    verify_layer2_startup_dependencies(environment="development")
