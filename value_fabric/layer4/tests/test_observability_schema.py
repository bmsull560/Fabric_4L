...
def test_layer4_lifecycle_schema_fields_present(caplog):
    ...
    stages = ["start", "checkpoint", "resume", "tool-call", "tool-result", "failure", "cancel", "completion"]

    with caplog.at_level(logging.INFO):
        for stage in stages:
            wrapper.emit(stage=stage, context=ctx, error_class="RuntimeError" if stage == "failure" else None, error_code="E_FAIL" if stage == "failure" else None)

    for rec in caplog.records:
        for field in ["request_id", "trace_id", "tenant_id", "workflow_id", "run_id", "provider_name"]:
